...
def save_fig(fig, title):
    '''Saves passed plot in "fig" directory'''
...
def plot_class_distribution(y, sub=''):
    '''Plots values distribution for parameter "y"'''
    distribution = np.unique(y, return_counts=True)
    # print standard deviation, mean, max, min and relative standard deviation of number of samples for each class
    print(pd.DataFrame(
    ...
    # plot two columns plot: first -> number of samples for each class ; second -> piechart with same information
...
def plot_features_info(series, title):
    '''Plots passed series avoiding starting from 0: starting from minimum value instead'''
...
def plot_importance(series, xlabel, title):
    '''Plots features' importance grouped by sensor'''
    data = pd.Series()
...
def plot_density_all(X, title_prefix):
    '''Plots the distribution of each sensor feature'''
    plots_per_page = 12
...
def plot_feature_score_correlation(feature_score_correlation, title):
    '''Plots feature score correlation between scores and hyperparameters for a model'''
    fig, ax = plt.subplots()
...
def plot_confusion_matrices(models, X, y, n_cols=3):
    '''Plots the confusion matrices of each model and for each subdatasets'''
...
def plot_accuracies(scores_table, n_cols=3):
    '''Plots a single graph for each set of models (grouped by 'dataset size')'''
...
def group_models(series: pd.DataFrame, models_names: list[str], subsets_sizes: list[str]):
    '''Takes "series" with models' score and groups them by dataset size'''
...
def plot_testing_accuracy(scores_table: pd.DataFrame, models_names: list[str], subsets_sizes: list[str]):
    '''Displays a single plot with test scores'''
...
def plot_losses(losses):
    '''Loss progression during epochs'''
    plt.figure()
